...
if ( ! class_exists( 'Chique_Customizer_Reset' ) ) {
	...
	final class Chique_Customizer_Reset {
		...
		public function customize_controls_print_scripts() {
			wp_enqueue_script( 'chique-customizer-reset', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'assets/js/customizer-reset.min.js', array( 'jquery' ), '20190207' );
			
			wp_localize_script( 'chique-customizer-reset', 'chiqueCustomizerReset', array(
				'reset'          => esc_html__( 'Reset', 'chique' ),
				'confirm'        => esc_html__( "Caution: Reset all settings to default. Process is irreversible.", 'chique' ),
				'nonce'          => array(
					'reset' => wp_create_nonce( 'chique-customizer-reset' ),
				),
				'resetSection'   => esc_html__( 'Reset section', 'chique' ),
				'confirmSection' => esc_html__( "Caution: Reset section settings to default. Process is irreversible.", 'chique' ),
			) );
		}
